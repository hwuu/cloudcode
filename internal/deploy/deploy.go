...
func (d *Deployer) printSuccess(state *config.State, cfg *DeployConfig) {
	...
	d.printf("首次登录需注册 Passkey:\n")
	d.printf("  1. 输入用户名密码后，点击 \"Register device\"\n")
	d.printf("  2. 运行 cloudcode otc 获取验证链接（不会发邮件）\n")
	d.printf("  3. 在浏览器中打开链接，完成 Passkey 注册\n\n")
	d.printf("提示:\n")
	d.printf("  - 查看状态:   cloudcode status\n")
	d.printf("  - SSH 登录:   cloudcode ssh\n")
	d.printf("  - 容器命令:   cloudcode exec devbox <cmd>\n")
	d.printf("  - 停机省钱:   cloudcode suspend\n")
	d.printf("  - 恢复运行:   cloudcode resume\n")
